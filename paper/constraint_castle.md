...
On September 8, 2026, OpenAI announced a claimed resolution of one branch of the Clay Mathematics Institute's Navier–Stokes Millennium Prize Problem: a finite-time blow-up construction for the three-dimensional incompressible Navier–Stokes equations, produced by roughly 10,000 coordinating AI agents over 88 hours, with a Lean formalization completed in a further 17 hours (Nature, 2026; Quanta Magazine, 2026).

A pre-written, ten-point skeptical critique of this result was available to us before any of it had been checked against the primary sources. Rather than accept or reject it wholesale, each checkable point was verified independently against the actual 166-page PDF and the actual public Lean repository:

- **Claimed corrupted PDF pages (pp. 20–21)**: false. The pages are a dense, coherent symbol-definition table with real cross-references to numbered propositions.
- **Claimed missing explicit formulas as a flaw**: not supported. Constructing a blow-up via an infinite hierarchy of corrections with parameters fixed by prior estimates, rather than closed-form elementary expressions, is standard practice in this area of PDE analysis, not evidence of invalidity on its own.
- **Claimed hidden `sorry` placeholders or unproven axioms in the Lean formalization**: false. The five `sorry` occurrences present in the repository are confined to a file the repository's own header describes as a standalone comparator copied from an unrelated project, explicitly excluded ("neither the proof root nor the submission imports this reference"); the proof directories actually used contain zero `sorry`s and zero custom axioms.
- **The theorem resolves the externally-forced variant of the problem, not the unforced case**: held up on verification, in a corrected form. The Clay Institute's official 2000 problem statement (Fefferman) does formally include a forced variant as one of four valid statements, so the claim that this is "not the original Clay problem" is imprecise. Independent mathematical commentary (Silvestre, quoted in Scientific American, 2026) confirms the more precise version of the objection: the forced variant is the easier branch, disconnected from the question the field actually wants answered, and removing the external force makes the constructed blow-up disappear.

Three of the four checkable objections in the pre-written critique were false or unsupported on direct verification; the fourth held up, but only once corrected to a more precise form than originally stated. The verification pass — reading the actual PDF pages, cloning and grepping the actual Lean repository, checking the actual official problem statement and independent mathematical commentary — took a small fraction of the resource expenditure of the 10,000-agent, 88-hour process it evaluated. This is not a claim that the underlying OpenAI result is wrong; it is a demonstration, on a live, high-profile example rather than an internal one, of the same mechanism reported in Sections 4.1–4.3, and of this paper's central claim in miniature: one well-verified pass, that keeps what holds and discards what does not, outperforms a much larger unverified effort — a single well-directed agent, not a swarm, is the stronger and more capable position.
...
15. *OpenAI claims huge maths breakthrough on a famed 'Millennium Problem'*, Nature, News, September 2026.
16. *AI Has Solved One of Math's $1 Million Millennium Prize Problems*, Quanta Magazine, September 8, 2026.
17. *Did OpenAI Solve the Wrong Navier-Stokes Problem?*, Scientific American, September 2026.
