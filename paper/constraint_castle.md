...
The castle mechanism described above operates inside a larger pipeline, which the human author formalized independently, over the course of this project, into four phases with four distinct success criteria — not four names for the same activity done with varying care, but four different questions, each answerable only by its own phase:

1. **Exploration** ("can this intuition become a working experiment?") — a divergent, low-cost phase run on a free-tier, large-context model, corresponding to Phase 1 (Draft) above and to the divergent pole of Guilford's (1956) and the Double Diamond's (Design Council UK, 2005) structure-of-intellect distinction already cited in Section 2.
2. **Verification** ("does the experiment actually implement what the literature states?") — a convergent phase run against the project's own retrieval system, corresponding to Phase 2 (Review) above.
