...
The castle mechanism described above operates inside a larger pipeline, structured as four phases, each with its own environment, tool, and success criterion — not four names for the same activity done with varying care, but four different questions, each answerable only by its own phase:

| Phase | Environment | Question | Output |
|---|---|---|---|
| 1. Exploration | free-tier, large-context model | Can this intuition become a working experiment? | prototype |
| 2. Verification | paid, retrieval-augmented model | Does the experiment actually implement what the literature states? | verified implementation |
| 3. Discovery | public repository | Can the result be documented and reproduced by someone else? | public, reproducible result |
| 4. Production | released library, under CI | Can it be integrated without breaking the software real users depend on? | release |

Phases 1 and 2 correspond directly to the divergent/convergent distinction already cited in Section 2 (Guilford, 1956; Design Council UK, 2005), and are deliberately assigned to two different models rather than to one model asked to do both: an agent instructed simultaneously to explore freely and to enforce every accumulated constraint receives a contradictory objective and satisfies neither well. Exploration therefore runs on a free, disposable, large-context model with no exposure to the constraint set; verification runs on a separate, retrieval-augmented model that treats the exploration output as already-finished work to be checked against the literature, not a sketch to improve.

Phases 3 and 4 each carry their own structural rules, independent of the constraint castle's content. Discovery requires every entry to be an independently runnable, versioned artifact with no publishing gate, so that a negative result costs nothing more than being documented with the same care as a positive one and left in place rather than deleted — Section 4.2 is one instance of exactly this rule in effect. Production requires the platform itself, not a human's memory, to refuse a merge until a named set of checks passes, and treats publication to a package index as a separate, human-confirmed step from everything that precedes it — Section 4.4 reports a failure of the assumption behind this rule, and its fix.

This four-way split is a specific instance of a general principle already named in software engineering: Boehm (1984) separates *verification* ("are we building the product right?") from *validation* ("are we building the right product?") specifically because a system can satisfy one while failing the other. The pipeline adds two further, independently failable questions on top of that pair — can the result be reproduced by someone outside the process (Discovery), and can it be integrated without regressing anything that already worked (Production) — because this project's own history (Section 4) shows failures at each of the four points independently: an idea that worked but was scientifically ungrounded, a scientifically sound result no one else could reproduce, and a fully verified, fully documented change that still broke a real installation once shipped (Section 4.4). Treating any one of these four checks as a stand-in for the others is exactly the failure mode Section 5 discusses under "conflating 'every test passed' with 'this is correct.'"
...
### 4.4 A packaging-list gap surfacing only in Discovery (`Dense-Evolution`)

Dense-Evolution's build configuration names the library's distributable sub-packages by hand rather than discovering them automatically. When a new sub-package (`dense_evolution.protocols`, covering three quantum-key-distribution protocol implementations) was added in an earlier pull request, the hand-maintained list was not updated in the same change. Every local test continued to pass, because local tests import directly from the source tree rather than from a built, installed artifact — the gap was invisible from inside the repository that produced it.

The gap surfaced only once an independent consumer installed the real published artifact: Dense-Evolution-Discovery's own continuous integration, which installs `dense-evolution` unpinned from the package index rather than from source, failed with `ModuleNotFoundError: No module named 'dense_evolution.protocols'` immediately after the affected version (8.2.2) was published. The fix was one line — adding the missing entry to the hand-maintained list — verified by comparing it directly against the real package directory tree (`find dense_evolution -name __init__.py`) and by inspecting the rebuilt artifact's contents before republishing. Because a package index does not allow a published version to be corrected in place, the fix shipped as a new version (8.2.3) rather than a repair of the broken one.

This is a direct instance of Production's own structural rule (Section 3.2): a hand-maintained list that names shipped components will drift the moment a new one is added without the list being updated in the same commit, and the drift is invisible to any check that runs against the source tree rather than the built artifact — caught here only because a second, independent repository (Discovery) installed the real published package rather than trusting the same tree that built it.

## 5. Discussion

**What generalizes, and what does not.** The mechanism — verify a specific failure directly, then convert it into exactly one permanent, specific artifact — generalizes across all four case studies and across two independently developed codebases. The constraints themselves, by design, do not generalize past the failure that produced each one; Section 4.2 shows directly that attempting to abstract past that boundary would have produced a false rule.
...
**Explicit limitation.** This paper reports case studies from two real projects, not a controlled experiment measuring an actual multi-agent, unverified baseline against the single-agent, constraint-accumulating condition described here on the same task. We make a mechanism claim, supported qualitatively by four independent instances, not a quantitative efficiency ratio. A controlled comparison — same task, same wall-clock or token budget, one run under an accumulated constraint set and one run as an unverified multi-agent baseline, with a real, pre-registered error metric — is the natural next step and is left to future work.
